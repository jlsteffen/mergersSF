...
Where $NUV-r$ is the color from SDSS's k-corrected absolute magnitude and $\mathcal{M}_i$ is the i-band magnitude from the NSA catalog. We further apply an \ewha\ $\ge$ 3\AA\ cut on galaxy spectra extracted from a 1.0 \reff\ aperture to remove any passive galaxies which may have passed the CMD cut \citep{Cid-Fernandes:2011}. We further use the BPT diagnostic \citep{Baldwin:1981} to remove galaxies with an AGN from our sample. We use the emission line ratios, log([O\textsc{III}]/H$\beta$) and log([N\textsc{II}]/H$\alpha$), extracted from a 1.3 kpc aperture along with the maximum starburst line from \citet{Kewley:2001} as the demarcation between star forming branch and the AGN branch.
...
Once the geometry of the galaxies are set, we can build the radial profiles. The spaxels are binned into radius increments of 0.2 R/\reff\ from 0.0$-$2.6 R/\reff. Within each radius bin we take the median of the specific star formation rate. The profiles' errors are the standard error of the mean of the data within each radius bin. We create one of these individual averaged radial profiles for every MaNGA galaxy. 
...
The radial extent of our profiles go out to 2.6 $R$/\reff; however,  67\% of the MaNGA sample is designed to cover only 1.5 $R$/\reff\ of the galaxy. Also, the MaNGA galaxies are not always centered on a spaxel center, so the 0$^{\rm th}$ spaxel may fall into the 0.2 $R$\reff\ bin. Because of these issues, the centers or the outskirts of the stacked profiles may only represent a small subset of the galaxies in that mass bin. To account for this, we ``trim" the stacked profiles where radii bins contain data for less than 50\% of the total set of galaxies. We represent the trimmed regions in our figures with dashed lines. In Figure \ref{fig:ssfr_prof}, this is shown in the centers of the lower mass galaxies, \logm\ $\le$ 10.0.
...
The profiles of the control galaxies are lower in their centers, which was previously seen in the MaNGA sample in \citet{Belfiore:2018}. The profiles of the paired galaxies flat across the whole galaxy accept for galaxies with masses above \logm\ $\ge$ 10.5 which show some central quenching. Because of this, it may be more accurate to describe the control galaxies as centrally suppressed or quenched in comparison to the paired galaxies which feature flat profiles of star formation. The difference in the pair and control profiles seems to indicate that the mechanism for quenching in galaxies is the depletion of gases in their centers. The gas inflows from the merger events can then refuel the centers of these quenched galaxies and allow new star formation to occur. 

When taking the difference between the pairs and controls, the paired profiles are centrally enhanced by $\sim$0.35 $\pm$ 0.1 dex which then falls to zero around 1.2 R/\reff. In the outskirts of the pairs beyond 1.2 R/\reff, the pairs feature lightly suppressed sSFR of 0.0$-$0.1 dex. All of the mass ranges except for the highest mass range, \logm\ $=$ 11.0$-$11.5, trend closely to the median profile. The most massive galaxies in the range, \logm\ $=$ 11.0$-$11.5, the enhancement to the sSFR within 0.5 R/\reff\ is higher than the median profile at 0.5$-$0.6 dex. The sSFR enhancement in the sample seems to mass independent except for the most massive galaxies which feature higher levels of enhancement.
...
\caption[]{Same as Figure \ref{fig:ssfr_mass} except the difference profiles are split by mass ratio. }
\label{fig:ssfr_dm}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\begin{figure*}
%\centering
%\includegraphics[width=\linewidth]{fig/ssfr_massdm.pdf}
%\caption[]{Same as the left hand panel of Figure \ref{fig:ssfr_mass} except primary companions are shown on the \textbf{Left} and secondary companions are shown on the \textbf{Right}. The \logm\ $=$ 11.0$-$11.5 mass bin is left out of left hand panel since it only contains two galaxies. }
%\label{fig:ssfr_massdm}
%\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In the top left of Figure \ref{fig:ssfr_dm}, we split the profiles by mass ratio in four bins; the primary and secondary companion of a major merger, $|\Delta$log(M)$|$ $<$ 0.5, and the primary and secondary companion of a minor merger, $|\Delta$log(M)$|$ $>$ 0.5. Regardless of the mass ratio, the centers of the paired galaxies are centrally enhanced, though major mergers have stronger enhancements in their centers by $\sim$0.2 dex. Secondary companions also feature slightly higher levels of enhancement than primary companions at the same mass ratios by $\sim$0.1 dex. 

The sSFR offsets in the outskirts of the galaxy pairs show a dependence on the mass ratio. Primary companions show a zero to positive enhancement, 0.0$-$ 0.4 dex, to the sSFR in their outskirts. Secondary companions feature a suppression to the sSFR in their outskirts of about 0.1$-$0.4 dex. 
...
From the mass ratio, we see two different effects. First, we see that the central enhancement to the sSFR is strongest for pairs with 1:1 mass ratios. Second, we see that secondary companions show steeper enhancement profiles with higher levels of sSFR enhancement in their centers and stronger sSFR suppression in their disks with respect to primary companions. It is interesting to note that the central enhancement to sSFR is dependent on both the mass ratio and the projected separation (which we will demonstrate in the next section) while the sSFR offsets in their outskirts primarily depends on whether the galaxy is the primary or secondary companion of the pair.
...
We look at the level of sSFR enhancement as a function of the projected separation between the two galaxies in Figure \ref{fig:ssfr_sep}. The $\Delta$log(sSFR) profiles show a clear gradient with the projected separation where close galaxy pairs show higher levels of sSFR enhancement compared to pairs with wide separations. The profiles in the galaxy pairs in the separation range, $r_{\rm p}$ $=$ 0.0$-$12.5, are generally $\sim$0.05$-$0.1 dex above the median and the profiles of the galaxy pairs in the separation range, $r_{\rm p}$ $=$ 37.5$-$50.0, are generally $\sim$0.05$-$0.1 dex below the median. 
...
\citet{Moreno:2015} used {\sc Gadget}-3, a smoothed particle hydrodynamics code, to study the spatial extend of the star formation enhancement. The merger simulations showed that the star formation enhancement was largely concentrated in the centers (R $<$ 1 kpc) of the paired galaxies and that the off-nuclear regions (1 $<$ R $<$ 10 kpc) showed a suppression to the star formation. \citet{Moreno:2015} also found that star formation offsets in lower mass secondary galaxies (in mergers with stellar mass ratios of 2.5:1) have a stronger radial dependence with respect to the more massive primary companion. This is in good agreement with our work, we found that secondary companions show moderately higher levels of sSFR enhancement in their centers in comparison to primary companions at the same mass ratio. Further, the outskirts of the primary companions feature no enhancement or suppression to their sSFR while the secondary companions show a strong suppression to the sSFR at large radii. 
...
In this work, we studied a sample of paired, star forming galaxies using the MaNGA IFS survey. We have made a sample of 175 star forming galaxy pairs along with a set of 1888 star forming control galaxies. We compared the radial profiles of the paired galaxies with those of the controls galaxies and found the following;

\begin{enumerate}
\item The sSFR is centrally enhanced within the inner 1.0 R/\reff\ by 0.25 $\pm$ 0.1 dex. 
\item The outskirts of the paired galaxies, R/\reff\ $=$ 1.0$-$2.5, show limited suppression in their disks of $\sim$0.1 dex.
...
\item Further, in paired galaxies with large mass ratios, the less massive galaxies experience higher levels of sSFR enhancement than the more massive galaxy in the pair by $\sim$0.1 dex.
\item The sSFR enhancement increases with closer projected separations where galaxies within 5$-$15 kpc have their sSFR enhanced by 0.3$-$0.4 which is consistent with previous single fiber studies. 
\item When splitting the sSFR enhancement profiles by stellar mass, mass ratio, or projected separation, the general shape of the enhancement profile is unchanged. While lower projected separations or mass ratios closer to 1:1 may raise the level of the central enhancement, the profiles still linearly fall to zero enhancement around 1.0 R/\reff.
\item The paired galaxies will experience a fractional mass growth rate of 1$-$6\%, where the growth in mass is largest in low mass galaxies.
