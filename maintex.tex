...
Once the geometries of the galaxies are set, we can build azimuthally averaged radial profiles. The spaxels are binned into radius increments of 0.2 \reff\ from 0.0$-$2.6 \reff. While the MaNGA sample is limited spatially to 1.5$-$2.5 \reff\, the spaxels along the semi-minor axis of a galaxy will be able to extend beyond that range because of the inclination angle, sin($i$). 
...
\end{equation}
where $M_{\rm target}$ is the stellar mass of the MaNGA target galaxy and $M_{\rm comp}$ is the stellar mass of the companion galaxy. We emphasize that the MaNGA target is not always the more massive component of the pair. We will define the more massive galaxy of a pair the primary companion and the less massive one the secondary companion.
...
\caption[]{The mean radial profile of $\Delta$log(sSFR) between pairs and controls of this work with the tailored control method ({\it Black}) compared against those of \citet{Pan:2019} ({\it Red}), a pair survey, and \citet{Thorp:2019} ({\it Blue}), a post-merger survey.}
\label{fig:prof_comp}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%The $\Delta$log(SFR) as a function of projected separation has been studied before with the SDSS survey \citep{Ellison:2008, Li:2008, Patton:2013, Scudder:2012, Bustamante:2020}. We compare the mentioned surveys with our own work in Figure \ref{fig:nuc_sep}. 
